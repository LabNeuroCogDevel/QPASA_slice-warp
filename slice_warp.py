...
class SliceWarp:
    ...
    def make_input(self):
        ...
        if re.match('.*\.nii(\.gz)?$', self.master.filename):
            nii = self.master.filename

        if nii:
            self.logfield.runcmd("3dcopy %s mprage1.nii" % nii)
        else:
    ...
    def resample(self, *args):
        ...
        if self.shouldresample.get():  # and min_d < 1:
            ...
            self.logfield.runcmd("3dcopy -overwrite mprage1.nii mprage1_res.nii.gz")
